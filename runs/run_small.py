"""One-command execution script for OutMind-Small (~125M parameters).

Optimized for consumer GPUs (RTX 3060/4060, Apple Silicon, Google Colab T4).
...
def main():
    parser = argparse.ArgumentParser(description="Run OutMind-Small (~125M) Pipeline")
    ...
    # Optimized defaults for Small (keeps peak VRAM < 5 GB on T4 / RTX 3060)
    default_pretrain = 15 if args.mode == "quick" else 2500
    default_sft = 8 if args.mode == "quick" else 1000
    default_bs = 2
    default_grad_accum = 1 if args.mode == "quick" else 2
